...
public class ItemLostPage extends Item {
    ...
    public ItemLostPage() {
        setUnlocalizedName(ModInformation.ID + ".page.lost");
    ...
    public static boolean bookHasPage(ItemStack bookStack, int book, int category, int entry, int pageNumber) {
        return bookStack.hasTagCompound() && bookStack.getTagCompound().getBoolean(book + ":" + category + ":" + entry + ":" + pageNumber);
    ...
    public static Object[] getPageCharacteristics(ItemStack pageStack) {
        ...
        try {
            ...
            objects[1] = GuideRegistry.getBook(NumberUtils.toInt(split[0])).getCategoryList().get(NumberUtils.toInt(split[1]));
            objects[2] = GuideRegistry.getBook(NumberUtils.toInt(split[0])).getCategoryList().get(NumberUtils.toInt(split[1])).entryList.get(NumberUtils.toInt(split[2]));
